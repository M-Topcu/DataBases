...
select isim, sum(urun_miktari) as toplam_meyve from manav group by isim;
...
select urun_adi, min(urun_miktari) as min_urun_mikt, max(urun_miktari) as max_urun_mikt from manav
where urun_adi is not null group by urun_adi order by max_urun_mikt;
...
select ulke, round(avg(maas),2) as ort_maas from personel group by ulke;

-- Maas ortalamasını bayanlar ve baylar olarak sorgulayınız
select cinsiyet, avg(maas) as ort_maas from personel group by cinsiyet;
